...
# 1. 페이지 설정
st.set_page_config(page_title="AAGIG - Game Insight Ground v10.0", layout="wide")

# 2. 스타일 시트 (v9.0 디자인 계승)
st.markdown("""
...
# 3. [진짜 엔진] 네이버 뉴스 기반 실시간 크롤링 (썸네일 포함)
...
def fetch_real_live_data():
        ...
        for art in articles[:20]:
            ...
            if title_el:
                ...
                if title not in seen:
                    thumb = img_el['src'] if img_el and 'src' in img_el.attrs else "https://via.placeholder.com/38?text=N"
                    # 네이버 이미지 썸네일 고화질화 (옵션)
                    thumb = thumb.replace('nf132_92', 'w640') 
                    
                    source = press_el.get_text(strip=True) if press_el else "뉴스"
                    tag = "tag-biz" if any(x in source for x in ["지디넷", "MTN", "딜사이트", "경제"]) else "tag-inven"
    ...
    # 데이터가 부족하면 예비 데이터로 채움
    if len(results) < 5:
        results = [{"title": "데이터 수집 중입니다...", "source": "AAGIG", "tag": "tag-global", "thumb": "", "views": 0, "cmts": 0}]
...
# --- 화면 렌더링 ---
st.markdown('<div class="main-logo-bar">AAGIG: Game Insight Ground v10.0</div>', unsafe_allow_html=True)
...
def draw_section(col, header, data_list):
    with col:
        ...
        for item in data_list[:8]:
            # onerror를 추가하여 이미지 로드 실패 시 대체 이미지 표시
            img_tag = f'<img src="{item["thumb"]}" onerror="this.src=\'https://via.placeholder.com/38?text=X\'">'
...
# 중간 배너
st.markdown('<div class="mid-banner">실시간 게임 산업 인사이트 통합 그라운드 | v10.0 이미지 엔진 가동</div>', unsafe_allow_html=True)
